...
function doPost(e) {
  try {
    ...
    if (!sheet) {
      ...
      sheet.appendRow(['תאריך','מספר הזמנה','שם לקוח','טלפון','אימייל','יישוב','כתובת','סוג הזמנה','הערות','אזור משלוח','דמי משלוח','אישור שיווקי','מוצרים','סה"כ',LINK_HEADER_NAME]);
    }
    ...
    const deliveryArea = data.deliveryArea || customer.deliveryArea || '';
    ...
    const rowData = {
      ...
      'דמי משלוח': deliveryFeeLabel || deliveryFee,
    ...
    const pdfFile = createOrderPdf_({orderNumber, customerName, phone, email, city, address, orderType, notes, deliveryArea, deliveryFee, deliveryFeeLabel, marketingConsent, items, total, date: now});
    if (WRITE_BACK_LINK) writeBackLink_(sheet, row, pdfFile.getUrl(), LINK_HEADER_NAME);
    sendNewOrderNotificationEmail_({orderNumber, customerName, phone, email, city, address, orderType, notes, deliveryArea, deliveryFee, deliveryFeeLabel, marketingConsent, items, total, date: now}, pdfFile);
...
function parseRequest_(e) {
  ...
  return {orderNumber:params.orderNumber, customerName:params.customerName || params.name, phone:params.phone, email:params.email, city:params.city, address:params.address, orderType:params.orderType, notes:params.notes, deliveryArea:params.deliveryArea, deliveryFee:params.deliveryFee, deliveryFeeLabel:params.deliveryFeeLabel, total:params.total, items:params.items ? JSON.parse(params.items) : []};
}
...
function createOrderPdf_({orderNumber, customerName, phone, email, city, address, orderType, notes, deliveryArea, deliveryFee, deliveryFeeLabel, marketingConsent, items, total, date}) {
  ...
  const customerTable = body.appendTable([
    ['שדה', 'פרטים'], ['שם לקוח', customerName || ''], ['טלפון', phone || ''], ['אימייל', email || ''], ['יישוב', city || ''], ['כתובת', address || ''], ['סוג הזמנה', orderType || ''], ['אזור משלוח', deliveryArea || '-'], ['דמי משלוח', deliveryFeeLabel || (deliveryFee === '' ? '-' : `₪${deliveryFee}`)], ['הערות', notes || ''], ['אישור שיווקי', marketingConsent ? 'כן' : 'לא']
  ]);
...
function sendNewOrderNotificationEmail_(order, pdfFile) {
  ...
  const body = [
    ...
    `יישוב: ${order.city || '-'}`, `כתובת: ${order.address || '-'}`, `סוג הזמנה: ${order.orderType || '-'}`, `אזור משלוח: ${order.deliveryArea || '-'}`, `דמי משלוח: ${order.deliveryFeeLabel || (order.deliveryFee === '' || order.deliveryFee == null ? '-' : `₪${order.deliveryFee}`)}`, `הערות: ${order.notes || '-'}`, `אישור שיווקי: ${order.marketingConsent ? 'כן' : 'לא'}`,
...
function setupCompassKfarSabaSheets() {
  ...
  if (orders.getLastRow() === 0) orders.appendRow(['תאריך','מספר הזמנה','שם לקוח','טלפון','אימייל','יישוב','כתובת','סוג הזמנה','הערות','אזור משלוח','דמי משלוח','אישור שיווקי','מוצרים','סה"כ',LINK_HEADER_NAME]);
}
